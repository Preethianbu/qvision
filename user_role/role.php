...
<th>Action</th>
      ...
      $assets_sql=$con->query("SELECT user_role_mapping.id as user_role_id,user_role_mapping.*,z_role_master.*,z_user_master.* FROM `user_role_mapping` INNER JOIN z_role_master ON user_role_mapping.rolemaster_id=z_role_master.id INNER join z_user_master ON user_role_mapping.employee_id=z_user_master.candidate_id where z_user_master.user_group_code<>'ROLE-R004' order by user_role_id DESC");
	   $i=1;
...
<script>
    ...
    $.ajax({
    type:"POST",
	url:"Qvision/user_role/role_add.php",
   ...
   function role_edit(v){
	...
	$.ajax({
	type:"POST",
	url:"Qvision/user_role/role_edit.php?id="+v,
    success:function(data)
	{
		$("#main_content").html(data);
	}
